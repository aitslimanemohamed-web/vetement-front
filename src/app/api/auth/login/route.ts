...
// Relais same-origin de la connexion (US-011) — même motif exact que
// /api/auth/register/route.ts (US-010) : le navigateur n'appelle jamais
// directement l'API NestJS, le jeton de session brut ne quitte jamais ce
// serveur.
...
export async function POST(request: NextRequest) {
  ...
  try {
    backendResponse = await fetch(`${apiUrl}/auth/login`, {
      method: 'POST',
  ...
  if (isSuccess && isRecord(backendBody) && isRecord(backendBody.session)) {
    ...
    const response = NextResponse.json({ status: 'LOGGED_IN', user: backendBody.user }, { status: 200, headers: NO_STORE });
  ...
  // Passthrough transparent de tous les cas d'erreur : VALIDATION_ERROR,
  // INVALID_CREDENTIALS, RATE_LIMITED (avec Retry-After), SERVICE_UNAVAILABLE,
  // INTERNAL_ERROR.
